...
This repository is a starter, not a production-ready identity or session adapter.
...
6. Replace the fail-closed service in `lib/eve/service.ts` with the authenticated `eve/client` adapter using Vercel OIDC and manual redirects.
